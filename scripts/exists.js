    ...
    function symbolize () {
        ...
        if ( !number ) {
            symbolization += "CHOOSE AN INTEGER, FUCKER";
                  ...
                  number !== "" ) {
            // as long as there's a number, then go ahead and generate
            // existential quantifiers binding that many variables
            for ( var i = 0; i < number; i++ ) {
                symbolization +=
                "<span class=\"logic exists\">∃</span>" +
                    makeVariable(i) + " ";
            }
            // now if there's a minimum number of things...
            if ( mode.value === "at-least" ||
                 mode.value === "exactly" ) {
				             // if symbolizing "at least one thing", there's going to be no
				             // closing paren, so no opening one either
                if ( !(mode.value === "at-least" &&
                       number < 2)) {
                    symbolization +=
                ...
                // it's actually essential that you do 'k==number' NOT
                // 'k===number'!
                for ( var j=0,k=0;
                      j < number && k < number;
                      k++,j=(k==number)?j+1:j,k=(k==number)?k=0:k ) {
                ...
                // remove the trailing "∧ "
                symbolization = (number > 1 ?
                                 symbolization.slice(0,symbolization.length - 2) :
                                 symbolization);
            }
            ...
            if ( mode.value === "exactly" ||
                ...
                symbolization +=
                     (mode.value === "exactly" && number > 1 ?
                      " ∧" : "") +
                         " <span class=\"logic forall\">∀</span>" +
                         forAllVar +
                         " <span class=\"parens\">(</span> ";

                for ( var l = 0; l < number; l++ ) {
                    symbolization += forAllVar +
                        "=" + makeVariable(l) + " ∨ ";
                }
                symbolization =
                         symbolization.slice(0,symbolization.length - 2);
                symbolization += (mode.value === "exactly" ?
                                  "<span class=\"parens\">)</span>" : "");
            }
            symbolization +=
                      ( !(mode.value === "at-least" && number < 2) ?
                        "<span class=\"parens\">)</span>" : "");
        }
